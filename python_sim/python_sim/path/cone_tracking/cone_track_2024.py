...
class PathPublisher(Node):
    def __init__(self):
        ...
        # Option
        self.waypoints_distance_thresold = (
            10.0  # waypoint가 서로 이어질 수 있는 최대 거리(float)
        )
        self.waypoints_thresold = (
            5  # 두번째와 세번째 waypoint가 고정되는 waypoints의 최소 개수(int)
        )
        ...
        self.yellow_sub = self.create_subscription(
            PointStamped, "yellow_odom", self.yellow_callback, 10
        )
        self.blue_sub = self.create_subscription(
            PointStamped, "blue_odom", self.blue_callback, 10
        )
        ...
        self.odom_sub = self.create_subscription(
            Odometry, "/odometry/navsat", self.odom_callback, 10
        )
    ...
    def yellow_callback(self, msg):
        ...
        for point in self.blue_points:  # 이미 감지된 파란색 콘과 겹칠 경우 제외
            ...
            if d < 0.3:
                ...
                first_blue_distances = cdist(
                    [[msg.point.x, msg.point.y]], self.blue_points
                )
                first_yellow_distances = cdist(
                    [[msg.point.x, msg.point.y]], self.yellow_points
                )
                ...
                copied_blue_points = np.delete(
                    copied_blue_points, first_blue_closest_idx, axis=0
                )
                copied_yellow_points = np.delete(
                    copied_yellow_points, first_yellow_closest_idx, axis=0
                )

                second_blue_distances = cdist(
                    [[msg.point.x, msg.point.y]], copied_blue_points
                )
                second_yellow_distances = cdist(
                    [[msg.point.x, msg.point.y]], copied_yellow_points
                )
                ...
                second_closest_yellow_cone = self.yellow_points[
                    second_yellow_closest_idx
                ]
                blue_p2l_distance = self.point_to_line_distance(
                    first_closest_blue_cone, second_closest_blue_cone, y_point
                )
                yellow_p2l_distance = self.point_to_line_distance(
                    first_closest_yellow_cone, second_closest_yellow_cone, y_point
                )
    ...
    def blue_callback(self, msg):
        ...
        for point in self.yellow_points:  # 이미 감지된 노란색 콘과 겹칠 경우 제외
            ...
            if d < 0.3:
                ...
                blue_p2l_distance = self.point_to_line_distance(
                    first_closest_blue_cone, second_closest_blue_cone, b_point
                )
                yellow_p2l_distance = self.point_to_line_distance(
                    first_closest_yellow_cone, second_closest_yellow_cone, b_point
                )
    ...
    def path_create(self):
        ...
        self.add_points_to_marker_array(
            marker_array, all_points, "points", 1.0, 0.0, 0.0
        )
        self.add_points_to_marker_array(
            marker_array, midpoints, "midpoints", 1.0, 1.0, 1.0
        )
        self.add_first_point_to_marker_array(
            marker_array, self.first_point, "first_point", 1.0, 0.0, 1.0
        )
        ...
        for simplex in simplex_indices:
            ...
            for i in range(len(simplex)):
                for j in range(i + 1, len(simplex)):
                    ...
                    if (
                        np.any(triangle_points[i] == self.blue_points)
                        and np.any(triangle_points[j] == self.yellow_points)
                    ) or (
                        np.any(triangle_points[j] == self.blue_points)
                        and np.any(triangle_points[i] == self.yellow_points)
                    ):
        ...
        if not self.flag_3:  # watpoints의 마지막 점이 결정되지 않았을 경우
            if self.flag_2:  # watpoints의 두번때, 세번째 점이 결정되었을 경우
                remaining_points = remaining_points[
                    ~np.all(remaining_points == self.first_point, axis=1)
                ]
                remaining_points = remaining_points[
                    ~np.all(remaining_points == self.second_point, axis=1)
                ]
                ...
                start_end_distance = np.hypot(
                    self.first_point[0] - waypoints[-1][0],
                    self.first_point[1] - waypoints[-1][1],
                )

                if (
                    start_end_distance < 3.5
                ):  # 시작점과 waypoints의 마지막 점 사이의 거리가 1.0 미만이면 path를 완성으로 판단
                    self.flag_3 = True
                    print("한바퀴")
                ...
                if (
                    len(waypoints) > self.waypoints_thresold + 2
                ):  # waypoint의 개수가 일정 개수 이상일 경우
