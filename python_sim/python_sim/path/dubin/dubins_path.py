...
def draw_point(point, arrow_length=0.5):
    ...
    plt.arrow(
        point[0],
        point[1],
        arrow_length * math.cos(point[2]),
        arrow_length * math.sin(point[2]),
        head_width=0.05,
    )
...
def test2():
    ...
    for i in range(12):
        ...
        dubins = DubinsPath(start, end, 3.0)
        dubins.calc_paths()
        ...
        draw_point(start)
        draw_point(end)
        xs, ys = DubinsPath.gen_path(start, path, 3.0)
        for i in range(3):
            plt.plot(xs[i], ys[i])
        plt.axis("equal")
    plt.show()
