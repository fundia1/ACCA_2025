    ...
    for m, l, i in zip(mode, lengths, range(len(mode))):
        ...
        while abs(pd) <= abs(l):
            ...
            px, py, pyaw, directions = interpolate(
                ind, pd, m, minr, ox, oy, oyaw, px, py, pyaw, directions
            )
            pd += d
        ...
        ind += 1
        px, py, pyaw, directions = interpolate(
            ind, l, m, minr, ox, oy, oyaw, px, py, pyaw, directions
        )
...
def get_paths(sx, sy, syaw, gx, gy, gyaw, minr, step_size):
    ...
    for path in paths:
        x, y, yaw, directions = generate_local_course(
            path.L, path.lengths, path.ctypes, minr, step_size / minr
        )
...
def reeds_shepp_planner(sx, sy, syaw, gx, gy, gyaw, minr, step_size):
...
def main():
    ...
    px, py, pyaw, mode, clen = reeds_shepp_planner(
    ...
    for i in range(len(pts) - 1):
        ...
        print(start_x, start_y, start_yaw, end_x, end_y, end_yaw)
        # if show_animation:
        # plt.cla()
