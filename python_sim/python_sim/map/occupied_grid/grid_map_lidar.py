...
class MapUpdater(Node):
    ...
    def lidar_callback(self, msg):
        ...
        for point in lidar_points:
            ...
            # 일정 높이 이상인 포인트만 처리 (예: z >= 0.0)
            if z <= -0.4:
                # Velodyne 기준 좌표계에서 fixed frame (map) 기준 좌표로 변환

                # Velodyne 기준에서 map 기준으로 회전 변환
                map_x, map_y = self.velodyne_to_map(x, y)

                self.update_map_with_pose(
                    (map_x, map_y), value=0, apply_inflation=False
                )

                # # map_x와 map_y를 해상도에 맞춰 정규화하여 맵 픽셀로 변환
                # map_x_pixel = int(map_x / self.resolution)
                # map_y_pixel = int(map_y / self.resolution)

                # # 맵 좌표가 유효한 범위 내에 있는지 확인
                # if (
                #     0 <= map_x_pixel < self.map_array.shape[1]
                #     and 0 <= map_y_pixel < self.map_array.shape[0]
                # ):
                # 맵 업데이트 함수 호출
                # self.update_map_with_pose((map_x_pixel, map_y_pixel), value=0)
        self.publish_map()
