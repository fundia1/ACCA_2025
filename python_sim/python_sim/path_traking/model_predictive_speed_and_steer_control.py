...
# mpc parameters
R = np.diag([0.01, 0.5])  # input cost matrix
...
MAX_DSTEER = np.deg2rad(500.0)  # maximum steering speed [rad/s]
...
def update_state(state, a, delta):
    ...
    if (
        abs(delta) > MAX_STEER / 2 and state.v < 1.0 and state.v > 0
    ):  # 속도가 일정 이하일 때
        state.v = 1.0
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    while MAX_TIME >= time:
        ...
        # Calculate MPC control signals
        oa, odelta, ox, oy, oyaw, ov = iterative_linear_mpc_control(
            xref, x0, dref, oa, odelta
        )

        # If valid control signals, update state
        di, ai = 0.0, 0.0
        ...
        # Yield current state and time for each step
        yield state.x, state.y, state.yaw, state.v, time, di, ai

        # Plot animation if needed
...
def get_switch_back_course(dl):
    ...
    # plt.plot(cx, cy, label="Path", color="b")

    # for i in range(0, len(cx), int(5.0)):
    #     x = cx[i]
    #     y = cy[i]
    #     yaw = cyaw[i]
    #     dx = math.cos(yaw)  # x방향
    #     dy = math.sin(yaw)  # y방향
    #     plt.arrow(x, y, dx, dy, head_width=1.0, head_length=2.0, fc="r", ec="r")

    # plt.title("Path with Arrows")
    # plt.xlabel("X")
    # plt.ylabel("Y")
    # plt.grid(True)
    # plt.legend()
...
def main():
    ...
    #     plt.subplots()
    #     plt.plot(t, a, "-b", label="acceleration")
    #     plt.grid(True)
    #     plt.xlabel("Time [s]")
    ...
    #     plt.subplots()
    #     plt.plot(t, d, "-g", label="steering angle")
    #     plt.grid(True)
    #     plt.xlabel("Time [s]")
...
def main2():
    ...
    for state_x, state_y, state_yaw, state_v, time, steer, accel in do_simulation(
    ...
    if not show_animation:  # pragma: no cover
        ...
        # Plotting the course and car's path
        plt.subplots()
        plt.plot(cx, cy, "-r", label="spline")
        plt.plot(x, y, "-g", label="tracking")
        plt.grid(True)
