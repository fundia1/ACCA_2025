...
class RobotNavigation:
    ...
    def initialize(self, start, goal, obstacles, x_min, x_max, y_min, y_max):
        ...
        if self.robot_position is None:  # 처음 실행할 때만 초기화
            self.robot_position = self.start.copy()

    def apf_fuzzy_navigation(self):
        """
        APF 기반의 장애물 회피 경로 생성
    ...
    def navigate(self, steps=100):
        ...
        robot_trajectory = [self.robot_position.copy()]
        ...
        for _ in range(steps):
            ...
            self.robot_position += move_vector * fuzzy_factor
            ...
            robot_trajectory.append(self.robot_position.copy())

        smoothed_trajectory = self.moving_average_path(robot_trajectory, window_size=5)

        return np.array(robot_trajectory), smoothed_trajectory

    def moving_average_path(self, trajectory, window_size=15):
        """경로를 부드럽게 만드는 함수"""
        smoothed_trajectory = []
...
def main():
    ...
    # 첫 번째 탐색
    robot_trajectory, smoothed_trajectory = robot_nav.navigate(steps=100)
    robot_nav.plot_trajectory(robot_trajectory, smoothed_trajectory)

    # 두 번째 탐색 (이전 위치에서 이어서 진행됨)
    robot_trajectory, smoothed_trajectory = robot_nav.navigate(steps=10)
    robot_nav.plot_trajectory(robot_trajectory, smoothed_trajectory)
