...
class Obstacle(Node):
    ...
    def global_path_callback(self, msg):
        ...
        if len(self.global_x) == 0 or self.global_x[0] != msg.poses[0].pose.position.x:
            ...
            # 2m 떨어진 사각형 좌표
            left_start = self.start + normal * 4  # 왼쪽 시작점
            right_start = self.start - normal * 4  # 오른쪽 시작점
            left_goal = self.goal + normal * 4  # 왼쪽 목표점
            right_goal = self.goal - normal * 4  # 오른쪽 목표점
    ...
    def add_obstacle(self, x, y, threshold=0.1):  # threshold=0.5
    ...
    def run_avoid_path(self):
        if len(self.start) != 0:
            self.apf.start = self.start
            self.apf.goal = self.goal
            self.apf.obstacles = self.obstacles
            self.apf.num_points = 3
            self.apf.x_min = self.x_min
            self.apf.x_max = self.x_max
            self.apf.y_min = self.y_min
            self.apf.y_max = self.y_max

            path_smooth = self.apf.make_apf_path()
