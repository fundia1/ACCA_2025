...
class Obstacle(Node):
    ...
    def add_obstacle(self, x, y, threshold=1.0):
        ...
        self.obstacles.sort(key=lambda obs: np.linalg.norm(self.start - obs))
    ...
    def run_uturn_path(self):
        if len(self.start) != 0:
            ...
            self.delau.obstacles = self.obstacles

            path = self.delau.make_trangle()
