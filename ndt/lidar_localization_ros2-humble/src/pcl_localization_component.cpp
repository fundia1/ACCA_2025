...
void PCLLocalization::odomKinematicReceived(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
{
  // RCLCPP_INFO(get_logger(), "odomKinematicReceived");

  // Ensure we have received both pcl_pose_ and GPS data
  if (!pcl_pose_ || !latest_gps_msg_) {
    RCLCPP_WARN(get_logger(), "Waiting for both pcl_pose and GPS data.");
    return;
  }

  // Check distance between localization/kinematic_state and pcl_pose
  double distance = computeDistance(pcl_pose_->pose.pose, msg->pose.pose);

  // GPS covariance values
  double gps_covariance_x = latest_gps_msg_->position_covariance[0];
  double gps_covariance_y = latest_gps_msg_->position_covariance[4];
  ...
  // Call initialPoseReceived with the new PoseWithCovarianceStamped message
  initialPoseReceived(pose_msg);

  RCLCPP_INFO(get_logger(), "Initial pose set using odometry!");
...
double PCLLocalization::computeDistance(const geometry_msgs::msg::Pose &pose1, const geometry_msgs::msg::Pose &pose2)
{
